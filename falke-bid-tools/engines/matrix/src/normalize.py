...
def _resolve_cell_state(item: LineItem) -> tuple[CellState, Optional[Decimal], str]:
    ...
    Priority order (highest wins):
      ...
      3. ALLOWANCE
      4. EXPLICIT_ZERO
      5. NULL_BLANK (amount is None, not explicit zero)
      6. AMOUNT (amount is set, positive)
    """
    if item.is_by_owner_others:
        return CellState.BY_OWNER_OTHERS, None, "BY OTHERS"
...
def _resolve_subtotal_cell(
    ...
    For ITEMIZED / PARTIAL_ITEMIZED: sum AMOUNT + EXPLICIT_ZERO + ALLOWANCE
      cells and compare to stated division_subtotal; flag discrepancy.
