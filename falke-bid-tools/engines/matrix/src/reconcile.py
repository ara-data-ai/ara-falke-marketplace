...
def _find_contractor_columns(ws, bids: list[NormalizedBid]) -> dict[str, int]:
    ...
    column (check 3 attribution).
    """
    out: dict[str, int] = {}
    expected_cols = [_col_start(i) for i in range(len(bids))]
...
def _check_sheet(
    ...
    name_to_col = _find_contractor_columns(ws, ordered_bids)
    subtotal_rows = _find_subtotal_rows(ws)
    grand_total_row = _find_footer_row(ws, _GRAND_TOTAL_KEY)
    # Resolve the row of every grand-total component (construction + fee rows).
    component_rows = {
        key: _find_footer_row(ws, key) for key in _GRAND_TOTAL_COMPONENT_KEYS
    }
    ...
    for bid in ordered_bids:
        ...
        for csi_code, _name in DIVISION_ROWS:
            ...
            if sub_row is None:
                ...
                continue
            written_sub = _as_decimal(ws.cell(row=sub_row, column=col).value)
            ...
            if sub_delta > TIEOUT_TOLERANCE:
                failures.append(_fail(
                    ...
                    f"written cell ({col_letter}{sub_row} = {_fmt(written_sub)}) does "
