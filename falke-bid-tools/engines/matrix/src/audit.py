    ...
    for bid in bids:
        ...
        for csi_code in CANONICAL_20:
            ...
            if div is None:
                ...
                if cross_bid_enabled and median_val > Decimal("0"):
            ...
            if cross_bid_enabled and subtotal_state == CellState.NULL_BLANK:
                items.append(AuditItem(
                    contractor_name=name,
                    division_csi=csi_code,
                    status=AuditStatus.YELLOW,
        ...
        if footer.grand_total.amount is None:
            ...
            construction_sub = footer.construction_subtotal.amount or Decimal("0")
            gl_amt = footer.general_liability_insurance.amount or Decimal("0")
            br_amt = footer.builders_risk_insurance.amount or Decimal("0")
            gc_fee = footer.gc_fee.amount or Decimal("0")
            computed_total = construction_sub + gl_amt + br_amt + gc_fee
