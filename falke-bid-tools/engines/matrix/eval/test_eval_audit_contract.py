...
Coverage (the brief's required minimum, mapped to Marvin's gold cases §7):
    ...
    GS-6  Robmar reclass (support) → YELLOW KNOWN_FIRM_RECLASSIFIED (positive foil to GS-3)
...
from src.firm_config import KnownFirmsConfig, load_known_firms  # noqa: E402
...
def _bid_gs6_robmar():
    """GS-6 (positive foil to GS-3): Robmar's SAME DIV 13 flooring line IS moved.
    Expect YELLOW KNOWN_FIRM_RECLASSIFIED (×2: flooring 13→09, dumpster 11→01)."""
    return _doc("Robmar Construction", [
        _div("DIV 13 00 00", "Special Construction", cost=CostStructure.ITEMIZED,
...
_MULTI_EXPECTATIONS = [
    ...
    Expect(
        case_id="GS-6 Robmar reclass (positive foil)",
        contractor="Robmar Construction",
...
def run_eval() -> list[CaseResult]:
    ...
    with tempfile.TemporaryDirectory() as d:
        ...
        multi_norm = compute_cross_bid_stats(
            [normalize_bid(doc) for doc in _MULTI_BIDS]
        )
        ...
        results.append(_grade(Expect(
            ...
            contractor="Robmar Restoration LLC",
