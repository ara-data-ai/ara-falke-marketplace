...
def _argv(**omit):
    ...
    matrix) and --baseline-confirmed, so the only stop this triggers is the
    MISSING-BAND stop in load_config — which fires BEFORE the matrix is opened.
    """
...
def test_missing_band_exits_nonzero_and_prints_stop(capsys):
    rc = main(_argv(band_low=True))
    assert rc == 2
