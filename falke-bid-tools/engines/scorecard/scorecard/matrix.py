...
def classify_subheader(value) -> Optional[str]:
    ...
    # $/SF family — the source typos this as "$/SXFX"; both start with "$/s".
    if n.startswith("$/s"):
...
class ParsedMatrix:
    ...
    # scan (Harbor's 3,030,000 lives here, not on a division row). None if the
...
class MatrixParser:
    ...
    # the validation matrix the quartet is 4 rows below the names (names row 8,
    ...
    def _detect_header_row(self, grid, max_row: int, max_col: int) -> int:
        ...
        if best is None:
            raise MatrixStructureError(
                ...
                "cells with the COST/$/SF sub-header quartet below it. Set "
                "matrix.header_row_hint in config to override."
            )
        return best[0]
    ...
    def _detect_blocks(self, grid, header_row: int, max_col: int, max_row: int
        ...
        if quartet is None:
            raise MatrixStructureError(
                ...
                f"{header_row} (scanned {self.QUARTET_SCAN_WINDOW} rows).")
    ...
    # block's sub-headers. The real the validation matrix wraps the two-word labels
        ...
        for c in range(start_col, upper):
            ...
            for rr in range(row_lo, row_hi + 1):
                ...
                if bk is not None and bk not in out:
                    out[bk] = c
                    break
        # require at least the COST column to consider this a block
    ...
    def _locate_grand_total(self, grid, max_row: int) -> Tuple[int, str]:
        ...
        scan that pre-markup per-bidder subtotal (where Harbor's 3,030,000
