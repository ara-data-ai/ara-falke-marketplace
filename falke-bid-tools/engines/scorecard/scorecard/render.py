...
from .scoring import CATEGORY_DISPLAY, CATEGORY_ORDER
...
def build_context(result: Dict, cfg: Config) -> Dict:
    ...
    # ---- Section E ----
    score_columns = [{"name": CATEGORY_DISPLAY[k],
                      "weight": f"{int(cfg.weights[k]*100)}%"}
                     for k in CATEGORY_ORDER]
    score_rows = []
    for b in bidders:
        scores = [fmt_score_cell(b["scores"].get(k)) for k in CATEGORY_ORDER]
